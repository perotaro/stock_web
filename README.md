...
- 売買ロジックや実データの詳細は非公開にし、公開リポジトリではWeb/API/設計ドキュメント/AI支援開発ルールを管理しています。
...
- 売買ロジックや実データの詳細を公開しないセキュリティ設計
- 単体テストと主要ルートのE2Eテスト

## 設計判断・トレードオフ

- 実データや売買ロジックの生成処理は公開せず、Web/API/設計ドキュメントのみ公開する方針にしました。
- 実DBがなくても画面確認できるよう、固定ダミーデータを返す簡易サーバーを用意しました。
- Web/API とバッチ処理側の本番対応は完了しており、公開トップ画面では実データと連携した集計サマリを表示しています。
...
現時点では、Web/API とバッチ処理側の本番対応は完了しています。
デプロイ構成は、当初想定していた AWS 構成どおりに構築しています。
日次データ更新やシグナル生成を担うバッチ処理は非公開リポジトリ側で管理しており、生成された実データを公開トップ画面の匿名集計サマリに連携しています。
...
バックエンドも実装しており、本番環境では非公開リポジトリ側で管理しているDB定義・バッチ処理・実データと連携します。
...
![AWS アーキテクチャ](docs/assets/guppy_aws_architecture.png)

公開リポジトリで管理している Web/API と、非公開リポジトリで管理しているバッチ処理・実データ生成処理を分離しています。
公開トップ画面では、バッチ処理で生成した実データの匿名集計サマリを API 経由で参照します。
...
現在の Web/API とバッチ処理のデプロイ構成は、以下の AWS 構成を基本としています。
